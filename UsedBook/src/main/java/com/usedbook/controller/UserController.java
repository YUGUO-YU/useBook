...
import com.usedbook.utils.EmailUtil;
...
public class UserController {
    ...
    public JSONObject getUser(@RequestBody User user, HttpServletResponse response) {
        ...
        try {
            ...
            if (dbUser.getPassWord().equals(StringTool.getMd5(user.getPassWord()))) {
                ...
                CurPool.curUserPool.put(dbUser.getId(),dbUser);
            } else {
                ret.put("data", false);
                ret.put("code", -1);
            ...
            try {
                ...
                if (user1.getPassWord().equals(StringTool.getMd5(user.getPassWord()))) {
                    ...
                    ret.put("code", 0);
                    ret.put("msg", "登录成功");
                    CurPool.curUserPool.put(user1.getUserName(),user1);
    ...
    public JSONObject loginOut(@PathParam(value = "userName") String userName,@PathParam(value = "userId") String userId, HttpSession session) {
        if ( userName!= null && !"".equals(userName)){
            CurPool.curUserPool.remove(userId);
            User dbUser = userService.qureyByUserName(userName);
            CurPool.sessionPool.remove(dbUser.getId());
            CurPool.webSockets.remove(dbUser.getId());
