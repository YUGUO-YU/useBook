    ...
    <nav class="clear">
      <div class="left">
        <div class="imgbox left">
          <img src="@/assets/img/booklogo.jpg" class="top_img" alt="">
        </div>
        <ul class="right">
          <router-link class="px16 pointer" to='/home' tag='li' replace><p @click="flagT();homelist()">首页</p>
          </router-link>
          <router-link class="px16 pointer" to='/release' tag='li' replace @click.native="xxx"><p>发布</p>
          </router-link>
          <router-link class="px16 pointer" to='/Car' tag='li' replace @click.native="xx"><p>收藏</p>
          </router-link>
          <router-link class="px16" style="cursor:help" to='/help' tag='li' replace @click.native="xxs"><p>订单</p></router-link>
        </ul>
      </div>
      <div class="right px14">
        <div class="px14" v-if="$store.state.img" style="padding-top: 20px">
          <el-button type="info" icon="el-icon-message" circle style="margin-left: 6px;" @click="getWebsocket()"></el-button>
    ...
    methods: {
      ...
      out() {
        this.$confirm('确定退出账号吗?', '提示', {
          ...
          this.$http.delete("http://127.0.0.1:8843/user/loginOut",{
            params:{
              userName: sessionStorage.getItem("user"),
